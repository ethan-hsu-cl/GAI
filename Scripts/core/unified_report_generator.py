...
class UnifiedReportGenerator:
    ...
    def add_media_universal(self, slide, placeholder_or_pos, media_path, is_video, slide_config, error_msg=None):
        """Universal media addition for all APIs"""
        ...
        if media_path and Path(media_path).exists():
            try:
                ...
                if is_video:
                    ...
                    slide.shapes.add_picture(str(media_path), fl, ft, sw, sh)
    ...
    def create_standard_media_pairs(self, folder: Path, ref_folder: Optional[Path], 
        ...
        src = {f.stem: f for f in folders['source'].iterdir() 
            ...
            for f in folders['generated'].iterdir():
                if f.suffix.lower() in gen_exts and file_pattern in f.name:
                    if self.api_name == 'nano_banana':
                        ...
                        out.setdefault(basename, []).append(f)
                    else:  # kling
        ...
        if use_comparison and ref_folder:
            ref_generated_folder = ref_folder / 'Generated_Output' if self.api_name == 'nano_banana' else ref_folder / 'Generated_Video'
                ...
                for f in ref_generated_folder.iterdir():
                    if self.api_name == 'nano_banana':
                        if f.suffix.lower() in {'.jpg', '.jpeg', '.png', '.webp'} and '_image_' in f.name:
                            basename = f.name.split('_image_')[0]
                            ref_files.setdefault(basename, []).append(f)
                    else:  # kling
                        if f.suffix.lower() in {'.mp4', '.mov', '.avi'} and '_generated' in f.name:
                            basename = f.stem.replace('_generated', '')
    ...
    def create_runway_media_pairs(self, folder: Path, ref_folder: Optional[Path], 
        ...
        for stem, meta_path in metadata_files.items():
            ...
            if ref_img_path:
                ...
                logger.warning(f"No valid source found for metadata: {stem}")
                continue

            # Handle reference comparisons
            ref_vid_path, ref_md = None, {}
            if use_comparison:
                r_base = stem.replace('runway_metadata_', '')
    ...
    def process_genvideo_batch(self, task: Dict) -> List[MediaPair]:
        ...
        for src_img in source_images:
            ...
            metadata_patterns = [
                metadata_folder / f"{basename}_{src_img.name}_metadata.json",  # Original pattern
                metadata_folder / f"{basename}_metadata.json",  # Simplified pattern
                metadata_folder / f"{src_img.stem}_metadata.json"  # Alternative pattern
            ]
            ...
            gen_img = generated_folder / f"{basename}.jpg"  # Default assumption
