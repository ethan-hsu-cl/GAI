"""Nano Banana report generator using refactored architecture."""
import sys
...
def collect_nano_banana_media_pairs(config_file: str) -> List[MediaPair]:
    """Collect media pairs from Nano Banana processing results."""
    ...
    for task in config.get('tasks', []):
        ...
        generated_folder = folder / 'Generated_Output'
        ...
        for source_file in source_folder.glob('*.jpg'):
            ...
            # ✅ CORRECT NANO BANANA PATTERN: {basename}_image_1.png, etc.
            generated_images = list(generated_folder.glob(f"{basename}_image_*.png"))
            ...
            media_pair = MediaPair(
                ...
                generated_paths=generated_images
            )
...
def main():
    """Generate Nano Banana report - SIMPLIFIED VERSION."""
    config_file = 'config/batch_nano_banana_config.json'
    
    # Collect media pairs
    media_pairs = collect_nano_banana_media_pairs(config_file)
    ...
    generator = ReportGenerator('nano_banana', config_file)
