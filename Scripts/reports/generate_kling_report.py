"""Kling report generator using refactored architecture."""
import sys
...
def collect_kling_media_pairs(config_file: str) -> List[MediaPair]:
    """Collect media pairs from Kling processing results."""
    ...
    for task in config.get('tasks', []):
        ...
        for source_file in source_folder.glob('*.jpg'):
            ...
            # ✅ CORRECT KLING PATTERN: {basename}_generated.mp4
...
def main():
    """Generate Kling report - SIMPLIFIED VERSION."""
    config_file = 'config/batch_config.json'
    
    # Collect media pairs
    media_pairs = collect_kling_media_pairs(config_file)
    ...
    generator = ReportGenerator('kling', config_file)
