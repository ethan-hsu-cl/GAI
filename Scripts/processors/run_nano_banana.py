"""Nano Banana processor using refactored architecture."""
import sys
...
def main():
    """Run Nano Banana processing."""
    logging.basicConfig(level=logging.INFO, format='%(message)s')
    
    # Load configuration
    config = ConfigManager.load_config('config/batch_nano_banana_config.json')
    if not config:
        print("Failed to load configuration")
        return
    
    # Create processor
    processor = ProcessorFactory.create_processor('nano_banana', config)
