"""GenVideo processor using refactored architecture."""
import sys
...
def main():
    """Run GenVideo processing."""
    logging.basicConfig(level=logging.INFO, format='%(message)s')
    
    # Load configuration
    config = ConfigManager.load_config('config/batch_genvideo_config.json')
    if not config:
        print("Failed to load configuration")
        return
    
    # Create processor
    processor = ProcessorFactory.create_processor('genvideo', config)
