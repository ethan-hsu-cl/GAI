...
class UnifiedReportGenerator:
    ...
    def _add_links_working_pattern(self, ppt: Presentation, task: Dict):
        """FIXED: Hyperlink creation using exact working patterns from Kling/Nano Banana with comprehensive logging"""
        if not ppt.slides:
            logger.warning("❌ No slides available for links")
            return

        slide = ppt.slides[0]
        logger.info("🔗 Starting hyperlink creation process")

        # STEP 1: Find existing info box using working pattern
        info_box = None
        for i, shape in enumerate(slide.shapes):
            if (hasattr(shape, 'text_frame') and shape.text_frame.text and 
                ...
                info_box = shape
                logger.info(f"✓ Found existing info box (shape {i}) with text: '{shape.text_frame.text[:50]}...'")
                break

        # STEP 2: Create new info box if not found (using working dimensions)
        if not info_box:
            info_box = slide.shapes.add_textbox(Cm(5), Cm(13), Cm(20), Cm(4))
            logger.info("✓ Created new info box at position (5, 13, 20, 4)")

        # STEP 3: Clear existing content
        info_box.text_frame.clear()
        logger.info("✓ Cleared info box content")

        # STEP 4: Get URLs from config with comprehensive logging
        testbed_url = self.config.get('testbed', f'http://192.168.4.3:8000/{self.api_name}/')
        design_link = task.get('design_link', '') or self.config.get('design_link', '')
        source_video_link = task.get('source_video_link', '') or self.config.get('source_video_link', '')

        logger.info(f"📋 URL Configuration:")
        logger.info(f"   Design Link: {'✓ ' + design_link[:50] + '...' if design_link else '❌ Not provided'}")
        logger.info(f"   Source Video Link: {'✓ ' + source_video_link[:50] + '...' if source_video_link else '❌ Not provided'}")
        logger.info(f"   Testbed URL: ✓ {testbed_url}")

        # STEP 5: Create links using EXACT working pattern from Kling optimized version
        links = [
            ("Design: ", "Link", design_link),
            ("Testbed: ", testbed_url, testbed_url),
            ("Source + Video: ", "Link", source_video_link)
        ]
        ...
        for i, (prefix, text, url) in enumerate(links):
            ...
            if url:  # Has URL - create hyperlink using exact Kling pattern
                logger.info(f"🔗 Creating hyperlink {i+1}: {prefix}")
                ...
                try:
                    ...
                    # Use URL for testbed, "Link" text for others (exact Kling logic)
                    r2.text = url if "Testbed:" in prefix else text
                    r2.font.size = Inches(24/72)
                    r2.hyperlink.address = url

                    para.alignment = PP_ALIGN.CENTER
                    logger.info(f"✅ Hyperlink created: {prefix}{r2.text} -> {url[:50]}...")

                except Exception as e:
                    logger.error(f"❌ Hyperlink creation failed for {prefix}: {e}")
                    # Fallback to plain text
                    para.text = f"{prefix}{text}"
                    para.font.size = Inches(24/72)
                    para.alignment = PP_ALIGN.CENTER

            else:  # No URL - plain text
                para.text = f"{prefix}{text}"
                para.font.size = Inches(24/72)
                para.alignment = PP_ALIGN.CENTER
                logger.info(f"📝 Added plain text: {prefix}{text} (no URL provided)")

        logger.info("🔗 Hyperlink creation process completed")

    def _create_title_slide_simple(self, ppt: Presentation, task: Dict, use_comparison: bool):
        """FIXED: Simplified title slide creation with working hyperlink pattern"""
        if not ppt.slides:
            logger.warning("❌ No slides found in presentation")
            return

        logger.info("📄 Starting title slide creation")
        ...
        if use_comparison and task.get('reference_folder'):
            ...
            logger.info(f"📋 Generated comparison title: {title}")
        else:
            ...
            if m:
                ...
                title = f"[{date}] {api_display}\n{folder_name}"
            logger.info(f"📋 Generated standard title: {title}")
        ...
        try:
            if ppt.slides and ppt.slides[0].shapes:
                ...
                logger.info(f"✅ Title successfully updated")
            else:
                logger.warning("⚠️ No shapes found in first slide")
        except Exception as e:
            logger.warning(f"⚠️ Could not update title slide: {e}")

        # FIXED: Add links using working pattern with comprehensive logging
        logger.info("🔗 Adding hyperlinks to title slide")
        self._add_links_working_pattern(ppt, task)

    def _load_report_definitions(self):
        """Load report definitions from api_definitions.json with flexible path"""
        definition_paths = [
