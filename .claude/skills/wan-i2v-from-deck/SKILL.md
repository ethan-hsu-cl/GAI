...
## This skill owns only the standalone I2V slides

One deck routinely carries slides for several modes at once, each labeled by its
`MODEL:` line. Take only the I2V ones and leave the rest to their own skill:

| Slide's `MODEL:` line | Config | Skill |
|---|---|---|
| `Wan 3.0 — Image-to-Video (I2V)`, standalone | `batch_wan_v3_i2v_config.yaml` | **this one** |
...
**An I2V slide titled `I2I→I2V …` is not a standalone style.** It is the second
step of the i2i2v pipeline, and its prompt is meant to run on the *generated*
frame from its I2I partner, not on a raw source photo. Only pull it in here if
the user explicitly wants that prompt benchmarked against raw sources too — and
if you do, say plainly in Step 8 that the same prompt now runs in two batches
from different inputs, so the duplication is a choice rather than an accident.

List the slides you skipped in Step 8 and name the skill that handles each.
...
- A `MODEL:` line naming the mode — use it to route the slide (see above).
...
- **The non-I2V slides you skipped**, and which skill handles each.
